            ...
            <div class="page-container">
                ...
                <div class="page-content-wrapper">
                    ...
                    <div class="page-content">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                            <c:if test="${curUser.role.code=='SUBADMIN' || curUser.role.code=='ADMIN'}"><ul class="table-top-tools"><li class="active"><a href="/report/under">未填写</a></li><li><span>|</span><a href="/report/deleted">已删除</a></li><li></ul></c:if>
										...
										<c:otherwise>
											<c:forEach items="${reportLogList}" var="reportLog" varStatus="status">
												<tr>
													...
													<td><c:if test="${reportLog.status==0 || reportLog.status==2}">未完成</c:if><c:if test="${reportLog.status=='1'}">已完成</c:if></td>
												    <c:if test="${curUser.role.code=='SUBADMIN' || curUser.role.code=='ADMIN'}"><td><button class="btn btn-outline red btn-sm" onclick="javascript:deleted(${reportLog.id});">删除</button></td></c:if>
												</tr>
											</c:forEach>
										</c:otherwise>
									</c:choose>
...
<![endif]-->
        ...
        <script type="text/javascript">
            var deleted = function(reportLogId){
        		//询问框
				layer.confirm('您确要删除这一条记录吗？', {
					btn : [ '确定', '取消' ]
				//按钮
					}, function() {
					$.ajax({
