...
var queryData = function(reportId,loadingYear,loadingMonth){
	...
	$.ajax({
		type : "get",
		url : "/summary/appealAnalysisJson",
		data : {
			reportId : reportId,
			loadingYear : loadingYear,
			loadingMonth : loadingMonth
		...
		success : function(data) {
			...
			if (data.code == 0) {
				layer.closeAll();
				if (data.reportLogList!=null&&data.reportLogList.length>0) {
					layer.msg('数据加载成功');
					...
					$.each(data.reportLogList,function(index,reportLog){
						dataContent = dataContent + "<tr>"+"<td><a href='/appeal_analysis/view/"+reportLog.id+"' tagert='_blank'>"+reportLog.title+"</a></td>"+"<td>"+reportLog.toUser.realname+"</td>";
						var status = '';
						var auditStatus = '';
