...
jQuery(document).ready(function() {
	$('#searchData').click(function() {
		...
		var loadingWeek = $('#selectWeek').val();
		...
		var dataContent = "";
		if (loadingYear != '' && loadingWeek != '') {
			queryData(reportId,loadingYear,loadingWeek);
			queryRemark(reportId,loadingYear,loadingWeek);
		}
	})
...
var queryData = function(reportId,loadingYear,loadingWeek){
	layer.msg('数据查询中', {
		time : 0, // 不自动关闭
		icon : 16,
		shade : 0.01
...
function queryRemark(reportId,loadingYear,loadingWeek){
	$.ajax({
		type : "get",
		url : "/summary/remark",
		data : {
