...
var queryJsonData = function (loadingYear){
	...
	$.ajax({
		...
		success : function(data) {
			layer.closeAll();
			if (data.virtualOperatorList!=null&&data.virtualOperatorList.length>0) {
				layer.msg('数据加载成功');
				$.each(data.virtualOperatorList,function(index,entity){
					dataContent = dataContent + "<tr><td  rowspan='2'><a href='javascript:void(0);' onclick='javascript:editData("+entity.id+");'>"+entity.year+"-"+entity.month+"</a></td><td>全国</td><td>"+converter(entity.field_11)+"</td><td>"+converter(entity.field_12)+"</td><td>"+converter(entity.field_13)+"</td><td>"+converter(entity.field_14)+"</td></tr>"+
					                            "<tr><td>本地</td><td>"+converter(entity.field_21)+"</td><td>"+converter(entity.field_22)+"</td><td>"+converter(entity.field_23)+"</td><td>"+converter(entity.field_24)+"</td></tr>";
				})
...
function editData(id){
	$.get("/gj//xnyys/detail?id="+id,function(data,status){
