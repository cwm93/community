...
var queryJsonData = function (loadingYear){
	...
	$.ajax({
		...
		success : function(data) {
			layer.closeAll();
			if (data.businessCardManagementlist!=null&&data.businessCardManagementlist.length>0) {
				layer.msg('数据加载成功');
				$.each(data.businessCardManagementlist,function(index,entity){
					var sum1 = entity.field_11+entity.field_21+entity.field_31;
					var sum2 = entity.field_12+entity.field_22+entity.field_32;
					var sum3 = (sum2*100/sum1).toFixed(2);
					dataContent = dataContent + "<tr><td  rowspan='4'><a href='javascript:void(0);' onclick='javascript:editData("+entity.id+");'>"+entity.year+"-"+entity.month+"</a></td><td>行业</td><td>"+sum1+"</td><td>"+sum2+"</td><td>"+sum3+"%</td></tr>" +
							                    "<tr><td>电信</td><td>"+converter(entity.field_11)+"</td><td>"+converter(entity.field_12)+"</td><td>"+converter(entity.field_13)+"%</td></tr>"+
...
function editData(id){
	$.get("/gj//hykgl/detail?id="+id,function(data,status){
