...
var queryJsonData = function (loadingYear){
	...
	$.ajax({
		...
		success : function(data) {
			layer.closeAll();
			if (data.resourceList!=null&&data.resourceList.length>0) {
				layer.msg('数据加载成功');
				$.each(data.resourceList,function(index,entity){
...
function editData(id){
	$.get("/gj//gdhmzy/detail?id="+id,function(data,status){
