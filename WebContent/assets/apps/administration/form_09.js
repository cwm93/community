...
var queryJsonData = function (loadingYear){
	...
	$.ajax({
		...
		success : function(data) {
			layer.closeAll();
			if (data.broadbandTariffList!=null&&data.broadbandTariffList.length>0) {
				layer.msg('数据加载成功');
				$.each(data.broadbandTariffList,function(index,entity){
					dataContent = dataContent + "<tr><td  rowspan='5'><a href='javascript:void(0);' onclick='javascript:editData("+entity.id+");'>"+entity.year+"-"+entity.month+"</a></td><td>上海电信</td><td>"+converter(entity.field_11)+"</td><td>"+converter(entity.field_12)+"</td><td>"+converter(entity.field_13)+"</td></tr>"+
					                            "<tr><td>上海移动</td><td>"+converter(entity.field_21)+"</td><td>"+converter(entity.field_22)+"</td><td>"+converter(entity.field_23)+"</td></tr>"+
					                            "<tr><td>上海联通</td><td>"+converter(entity.field_31)+"</td><td>"+converter(entity.field_32)+"</td><td>"+converter(entity.field_33)+"</td></tr>"+
					                            "<tr><td>长城宽带</td><td>"+converter(entity.field_41)+"</td><td>"+converter(entity.field_42)+"</td><td>"+converter(entity.field_43)+"</td></tr>"+
					                            "<tr><td>东方有线</td><td>"+converter(entity.field_51)+"</td><td>"+converter(entity.field_52)+"</td><td>"+converter(entity.field_53)+"</td></tr>";
				})
...
function editData(id){
	$.get("/gj//kdzf/detail?id="+id,function(data,status){
		  ...
		  $('#field_52').val(data.entity.field_52);
		  $('#field_53').val(data.entity.field_53);
		  $("#deleteBtn").show();
		  $('#add-form').modal('show');
